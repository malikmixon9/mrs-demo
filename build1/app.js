...
function renderAddClient(c) {
  setTitle("Add Client");
  const coachOpts = COACHES.map(n => `<option>${n}</option>`).join("");
  const svcOpts = SERVICE_TYPES.map(t => `<option>${t}</option>`).join("");
  const today = new Date().toISOString().slice(0, 10);
  c.innerHTML = `
    <div class="panel">
      <div class="panel-head"><h3>New client record</h3><span class="locked" style="font-style:normal">from the MRS referral packet</span></div>
      <div class="log-form" style="border-top:0;background:#fff;">
        <div class="grid2">
          <div><label>Client full name</label><input type="text" id="n_name" placeholder="First and last name"></div>
          <div><label>Authorization number</label><input type="text" id="n_auth" placeholder="MRS-000000"></div>
        </div>
        <div class="grid2">
          <div><label>SSN</label><input type="text" id="n_ssn" placeholder="000-00-0000"></div>
          <div><label>Primary disability / barrier</label><input type="text" id="n_dis" placeholder="From referral"></div>
        </div>
        <div class="grid2">
          <div><label>MRS counselor</label><input type="text" id="n_couns" placeholder="Counselor name"></div>
          <div><label>Counselor email</label><input type="text" id="n_email" placeholder="name@michigan.gov"></div>
        </div>
        <div class="grid2">
          <div><label>Service type</label><select id="n_svc">${svcOpts}</select></div>
          <div><label>Vocational goal</label><input type="text" id="n_goal" placeholder="e.g. Retail, Data Entry"></div>
        </div>
        <div class="grid2">
          <div><label>Authorization start</label><input type="date" id="n_start" value="${today}"></div>
          <div><label>Authorization end</label><input type="date" id="n_end"></div>
        </div>
        <div class="grid2">
          <div><label>Authorized hours</label><input type="number" id="n_hours" min="1" value="24"></div>
          <div><label>Assign coach</label><select id="n_coach">${coachOpts}</select></div>
        </div>
        <div><button class="btn primary" id="n_save">Create client record</button></div>
        <div class="locked" style="font-style:normal">Coach accounts are created the same way from the team screen in the full build. Demo data only.</div>
      </div>
    </div>`;
  el("n_save").onclick = () => {
    const name = el("n_name").value.trim();
    if (!name) { el("n_name").focus(); return; }
    const end = el("n_end").value || new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10);
    CLIENTS.push({
      id: "c" + (CLIENTS.length + 1) + "_" + CLIENTS.length,
      name: name,
      authNumber: el("n_auth").value.trim() || "MRS-000000",
      ssn: el("n_ssn").value.trim() || "000-00-0000",
      counselor: el("n_couns").value.trim() || "(counselor)",
      counselorEmail: el("n_email").value.trim() || "(email)",
      serviceType: el("n_svc").value,
      authStart: el("n_start").value || new Date().toISOString().slice(0, 10),
      authEnd: end,
      authorizedHours: Number(el("n_hours").value || 24),
      coach: el("n_coach").value,
      disability: el("n_dis").value.trim() || "(from referral)",
      goal: el("n_goal").value.trim() || "(goal)",
      status: "New",
      progressLog: [],
      placementLog: [],
      leads: []
    });
    audit("Created client record", name + " assigned to " + el("n_coach").value);
    go("tracker");
  };
}
...
function renderReminders(c) {
  ...
  c.innerHTML = `
    ...
    <p class="locked">These are derived automatically from the data already in the system: authorization deadlines, logged next steps, and recent placement activity. Nothing here is hand-kept.${isCoach ? "" : ""}</p>`;
  wireClientRows();
  wireDraftButtons();
}
